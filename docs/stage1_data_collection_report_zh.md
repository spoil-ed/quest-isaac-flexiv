...
- 新增 `flexiv_data_collection/` 自包含包，不依赖仓库外的 Unitree/遥操作工程路径。
...
- 新增三层配置：environment config 管仓库相对路径和外部运行时入口，scene config 管 Rizon4/相机，pipeline config 管 gateway/record/convert/validation。scene serial 不提供机器默认值，须按 README 通过 CLI 填写；脚本会校验 sample serial 与当前参数一致。
...
```bash
python3 scripts/run_stage1_data_collection_smoke.py
```
