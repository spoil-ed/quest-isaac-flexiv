...
class TeleVuer:
    ...
    async def on_controller_move(self, event, session, fps=60):
        ...
        try:
            ...
            try:
                print(f"[TeleVuer] CONTROLLER_MOVE value keys: {list(event.value.keys())}", flush=True)
                print(f"[TeleVuer] CONTROLLER_MOVE value: {event.value}", flush=True)
            except Exception:
                pass
            traceback.print_exc()
