...
当前先用 `/World/TargetFrame` 这个 XYZ 箭头目标替代 Quest pose 验证链路：
...
验收目标：拖动 XYZ 箭头目标或移动 Quest 手柄时，Rizon4 在 Isaac 中实时跟随；同时能记录目标位姿、`q`、`dq`、`target_drives`，后续用于轨迹采集、LeRobot 转换和 replay。
