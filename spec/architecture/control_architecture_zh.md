...
5. `joint_initializing`：切换 `NRT_JOINT_POSITION`，先以切换后的当前 q 调用 `SendJointPosition()` 建立连续基准，再调用 `SendJointPosition(init_q_left, init_q_right)`；由 runtime 生成关节轨迹，等待位置和速度连续稳定。
...
reset 不调用 Isaac `world.reset()`，不瞬时写 articulation，也不使用旧 TCP target pose 代替任务 `initial_q`。任一步失败都发布 `reset_failed` 和错误原因；发生回程失败时保持用户控制关闭及场景物体碰撞暂时禁用，使下一次按 `r` 可以再次脱离接触。机械臂 articulation 自身的碰撞始终保持启用。recorder 不因一次 reset 失败退出，而是暂停当前 episode 并等待重试。同一序号不自动重试，下一次按 `r` 产生更高序号后才重新执行完整状态机。
...
5. 左右手、左右目标、左右标定和左右 limiter 完全独立；DRDK 只原子同步传输同周期的两侧目标。
