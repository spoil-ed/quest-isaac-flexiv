...
双手共享坐标系默认采用宽松锁存：两只控制器均有 OpenXR tracking、水平投影间距至少 0.05 m 且双手 squeeze 同时达到 0.15 时，立即以左右手柄连线定义横向轴，以两手柄平均前向在横向正交平面内的投影定义前向轴，并用叉乘生成向上的第三轴。40 cm 间距和方向误差只用于 `print.sh` 诊断，不阻止锁存。首次锁存、松开后再次按下都以当下手柄和 TCP 作为相对零点，第一帧位移严格为零；松开保持最后目标，不回初始位姿。reset 后仍要求先释放再双手 squeeze，防止旧按键状态自动重锁存。
...
环境接触保护使用 `RobotPair.SetMaxContactWrench((left_limit, right_limit))`，默认每侧 `[30,30,30,5,5,5]`（前三项 N，后三项 Nm），且每次重新进入 `NRT_CARTESIAN_MOTION_FORCE` 都必须重设。该接口负责 runtime 内层接触 wrench 调节，不等同于停止目标生成。streamer 因此还读取每侧 `states().tcp_wrench`，独立运行带迟滞的目标冻结器：单样本达到上限 90% 时锁存该侧实际 TCP 为 hold pose，另一侧不受影响；六个分量全部降到上限 55% 以下并稳定 0.12 s 后解除。解除时以最新原始输入为输入零点、hold pose 为输出零点，后续位置增量和四元数旋转增量都从该零点继续，禁止追赶接触期间累积的绝对目标。reset 或重新锁存 Cartesian reference 时必须清空两侧冻结、计数和重映射状态。`tcp_wrench` 与 `contact_frozen` 随 status 数据发布，供诊断确认实际仿真反馈是否有效。

关节力矩前置保护封装在 `JointTorqueGuard`，不介入 2 kHz 力矩解算。streamer 从 `RobotPair.info().tau_max` 读取左右每关节软件上限，并在每个 target 周期读取 `states().tau`、`tau_dot` 和 `tau_ext`。每个关节还以连续 `tau` 样本计算并滤波有限差分斜率；预测时在 runtime `tau_dot` 和本地斜率之间选择能产生更大绝对预测力矩的一项，避免仿真 runtime 始终返回零 `tau_dot` 时丢失提前量。单关节风险比定义为 `max(|tau|, |tau_ext|, |tau + tau_dot_effective * 0.025|) / tau_max`。任一比值达到 0.72 即停止采用该侧新 Quest 目标，并通过 `SendCartesianMotionForce()` 回退至至少 0.05 s 前发送的最近安全 target；另一侧继续独立工作。全部关节风险比低于 0.55 并持续 0.15 s 后解除，解除点以最新保护输入为输入零点、回退 target 为输出零点，位置和姿态都继续相对映射。reset 或重新锁存 Cartesian reference 必须清空历史、冻结和重映射，并以当前 reference pose 建立第一条安全历史。status 必须发布 `joint_tau`、`joint_tau_dot`、`joint_tau_ext`、`joint_tau_max`、`joint_torque_ratio` 与 `joint_torque_frozen`。
...
未标定时，双手 pose 持续发布给 `print.sh` 和 Web 面板，但不发布 `rizon4_quest_target.v1`。当前启动方案启用严格一致性门控：两只手柄都有 tracking、`SPACING` 与 `DIRECTION` 连续通过 0.25 s，并同时按下双手 `squeeze`，才允许锁定共享坐标系。初始化双臂 TCP 间距为 0.40 m 且朝向平行，因此手柄要求 0.40±0.01 m，并要求两手柄水平指向相互平行且分别垂直于实时双手连线，最大误差 15°。这保证锁定瞬间手柄相对位姿与双臂 TCP 相对位姿一致；锁定后的同尺度相对增量继续保持双手距离、双臂距离以及相对方向的对应关系。

坐标系确认和机械臂接管使用同一次双 squeeze。`SPACING` 与 `DIRECTION` 连续通过 0.25 s 后，publisher 在当前帧锁定共享坐标系；左右 mapper 同时以该帧手柄 pose 建立零点，Isaac 同时以当前左右 TCP 建立输出锚点，因此第一条目标的平移和旋转增量必须严格为零。用户无需松开或二次按压，继续保持 squeeze 并移动即可相对跟随。
...
```text
e_y = normalize((u_left-u_right)_xy)
e_x = normalize(project_xy(f_left + f_right) - dot(project_xy(f_left + f_right), e_y) * e_y)
e_z = normalize(cross(e_x, e_y))
C   = [e_x^T; e_y^T; e_z^T]
```

若平均前向与手柄连线近似平行，`e_x` 使用水平面内与 `e_y` 垂直的确定性后备方向。必要时同时反转 `e_y/e_z`，保证 `e_z` 朝上且 `C` 为右手正交基。两手中点是标定坐标系的概念原点；由于控制只使用每只手的相对增量，平移原点会在相减时抵消。`C` 在确认瞬间锁存，松开 squeeze、暂停或重新接管都不得改变；协调 reset 或重启 publisher 才会使它失效。
...
两只手柄 pose/tracking 任一缺失时两行均为 `WAIT`，数值超限为 `FAIL`，通过为绿色 `PASS`。当前 `start.sh` 固定启用 `gate=STRICT`：只有两行都通过时，双手 squeeze 才能把共享坐标系从 `ALIGN` 经 `CONFIRMING` 锁定为 `LOCKED`；任一项失败都不得发布机械臂目标。`print.sh` 和 Web 面板只显示状态，真正的锁存由 Quest publisher 完成；完整机器人与映射诊断只在 `--verbose` 中展示。
