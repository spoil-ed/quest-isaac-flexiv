...
DRDK 初始化顺序固定为：两个 runtime operational、切换 NRT 模式、读取两臂当前 `q`、调用 `SetNullSpacePosture((q_left, q_right))`、配置零空间参考跟踪权重、锁存两臂当前 TCP，最后同时发布 ready。显式任务姿态可覆盖当前 `q`，但必须在该 runtime 的关节限位内。
...
12. DRDK 切换 NRT 模式后必须初始化两臂零空间参考；重新进入该模式时必须重新初始化。
