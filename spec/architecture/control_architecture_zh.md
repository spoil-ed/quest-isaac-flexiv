...
启动器从唯一 pipeline 的 `control.drdk` 读取 DRDK 安全参数。它可在 `RobotPair` 创建后、初始化轨迹开始前显式创建官方 DRDK `SelfCollisionMonitor`，但当前 pipeline 默认关闭；`SELF_COLLISION_MONITOR=true/false` 只用于临时覆盖。启用时，它使用 scene `robots[].position` 作为两侧 base 在共同世界坐标中的平移，以 10 ms 周期检查两臂几何、保留 0.05 m 最小距离；默认不跳过任何 link。距离不足时 DRDK 无视当前任务和控制模式停止双臂，streamer 锁存 `self_collision_stopped`，停止转发 Quest 目标，直到操作者排除近距离条件并发起协调 reset。该模块只覆盖左右机械臂之间的自碰撞，不覆盖桌面、工件等场景接触；监视器是独立生命周期对象，`RobotPair` 因断连重建时必须随之重建，进程退出时必须停止。

环境接触保护使用 `RobotPair.SetMaxContactWrench((left_limit, right_limit))`，默认每侧 `[20,20,20,3,3,3]`（前三项 N，后三项 Nm），且每次重新进入 `NRT_CARTESIAN_MOTION_FORCE` 都必须重设。该接口负责 runtime 内层接触 wrench 调节，不等同于停止目标生成。streamer 因此还读取每侧 `states().tcp_wrench`，独立运行带迟滞的目标冻结器：连续 3 个样本达到上限 95% 时锁存该侧实际 TCP 为 hold pose，另一侧不受影响；六个分量全部降到上限 70% 以下并稳定 0.30 s 后解除。解除时以最新原始输入为输入零点、hold pose 为输出零点，后续位置增量和四元数旋转增量都从该零点继续，禁止追赶接触期间累积的绝对目标。reset 或重新锁存 Cartesian reference 时必须清空两侧冻结、计数和重映射状态。`tcp_wrench` 与 `contact_frozen` 随 status 数据发布，供诊断确认实际仿真反馈是否有效。

关节力矩前置保护封装在 `JointTorqueGuard`，不介入 2 kHz 力矩解算。streamer 从 `RobotPair.info().tau_max` 读取左右每关节软件上限，并在每个 target 周期读取 `states().tau`、`tau_dot` 和 `tau_ext`；单关节风险比定义为 `max(|tau|, |tau_ext|, |tau + tau_dot * prediction_horizon|) / tau_max`。默认预测窗口 0.02 s，任一比值达到 0.85 即停止采用该侧新 Quest 目标，并通过 `SendCartesianMotionForce()` 回退至至少 0.10 s 前发送的最近安全 target；另一侧继续独立工作。全部关节风险比低于 0.70 并持续 0.30 s 后解除，解除点以最新保护输入为输入零点、回退 target 为输出零点，位置和姿态都继续相对映射。reset 或重新锁存 Cartesian reference 必须清空历史、冻结和重映射，并以当前 reference pose 建立第一条安全历史。status 必须发布 `joint_tau`、`joint_tau_dot`、`joint_tau_ext`、`joint_tau_max`、`joint_torque_ratio` 与 `joint_torque_frozen`。

唯一 pipeline 的 `control.joint_effort_limits_nm` 把两套 Isaac articulation 的 J1..J7 最大 effort 设置为 `[150,150,80,80,49,49,49] Nm`，并在初始化时回读确认；该值与 Studio 模拟 Rizon4 已有的 safety-function 配置一致。`RobotInfo.tau_max` 仍提供更低的额定保护基准，供 85% 前置回退使用；Isaac 上限不能代替该回退或更低的 TCP 接触 wrench 限制。
...
必要时同时反转 `e_y/e_z`，保证 `e_z` 朝上且 `C` 为右手正交基。两手中点是标定坐标系的概念原点；由于控制只使用每只手的相对增量，平移原点会在相减时抵消。`C` 在确认瞬间锁存，松开 squeeze、暂停或重新接管都不得改变；协调 reset 或重启 publisher 才会使它失效。

协调 reset 开始后，Isaac 通过独立的本机 UDP 标定失效通知，把同一个 `reset_seq` 重复发送给 Quest publisher。publisher 按序号去重后清除共享旋转和左右相对 mapper，且必须先观测到旧的双 squeeze 已释放；随后只有再次同时按住两手 squeeze、并持续通过距离和方向检查，才能重新锁定坐标系。在重新标定前只发布原始 Quest 诊断和夹爪输入，不发布机械臂跟随目标。
