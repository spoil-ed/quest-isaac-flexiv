...
## 4. 在 Isaac Sim 环境安装 Quest publisher 依赖

Quest publisher 默认与 Isaac Sim 共用 `isaacsim` conda 环境：

```bash
conda activate isaacsim
python -m pip install 'vuer==0.0.60' 'params-proto==2.13.2' 'aiohttp-cors>=0.7'
```

不要安装 `vuer[all]`：它会把 `aiohttp` 固定到与 Isaac Sim 6.0.1 不兼容的旧版本。`third_party/televuer` 源码已随仓库提交，publisher 会自动加入其路径，不需要单独安装 TeleVuer。
...
- `No module named vuer`：确认使用 `isaacsim` 环境，并按第 4 节安装 Quest publisher 依赖。
